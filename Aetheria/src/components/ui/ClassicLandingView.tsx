...
/**
 * legacyCopyEmail — execCommand('copy') fallback for contexts where the
 * async Clipboard API is unavailable (older Safari, non-secure origins).
 * Returns whether the copy succeeded so the caller can toast accordingly.
 */
...
/**
 * ClassicLandingView — the classic (non-WebGL) resume experience.
 *
 * A single long-scroll editorial landing: sticky desktop nav + mobile chip
 * nav with scroll-spy, hero (TypedBio / RotatingRole), projects, about,
 * education, skills, certifications and contacts sections. Shares the
 * day/night engine with the 3D view: the page background/aura mirror the
 * live `dayNightState` sky, and the header hosts the same TimeOfDaySlider.
 */
