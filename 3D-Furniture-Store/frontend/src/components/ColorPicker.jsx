...
export default function ColorPicker() {
  ...
  if (selectedObjectId) {
    ...
    if (obj && obj.colors && obj.colors[selectedObjectPart]) {
    ...
    <div style={{ position: 'absolute', right: '40px', top: '50%', transform: 'translateY(-50%)', width: '320px', zIndex: 20 }} className="glass-panel">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 20px', borderBottom: '1px solid var(--color-glass-border)' }}>
        <h3 style={{ margin: 0, fontSize: '1.2rem', textTransform: 'capitalize' }}>
          {activePart.replace(/_/g, ' ')}
        </h3>
        <button onClick={() => { setSelectedPart(null); setSelectedObjectPart(null); }} style={{ background: 'transparent', border: 'none', color: 'var(--color-text)', cursor: 'pointer', transition: 'color 0.2s' }} onMouseEnter={e => e.target.style.color = '#ff6b6b'} onMouseLeave={e => e.target.style.color = 'var(--color-text)'}>
          <X size={20} />
        </button>
      </div>
