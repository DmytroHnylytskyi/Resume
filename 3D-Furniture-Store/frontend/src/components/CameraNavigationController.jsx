...
/**
 * @file CameraNavigationController.jsx
...
export default function CameraNavigationController() {
  /** @type {React.RefObject<import('@react-three/drei').CameraControls>} Ref to underlying CameraControls instance */
  ...
  /** @type {React.RefObject<Object.<string, boolean>>} Active key state map */
  const keys = useRef({});

  /** @type {React.RefObject<THREE.Vector3>} Velocity vector for camera movement interpolation */
  const velocity = useRef(new THREE.Vector3());

  /** @type {React.RefObject<THREE.Vector3>} Target velocity vector derived from user key inputs */
  const targetVelocity = useRef(new THREE.Vector3());

  /**
   * Effect hook to focus camera on selected object with smooth box padding.
   * Traverses scene hierarchy to find mesh matching selectedObjectId and triggers fitToBox framing.
   */
  useEffect(() => {
    ...
    if (targetGroup && controlsRef.current) {
      try {
        targetGroup.updateMatrixWorld(true);
        controlsRef.current.fitToBox(targetGroup, true, {
          paddingLeft: 0.4,
          paddingRight: 0.4,
          paddingTop: 0.4,
          paddingBottom: 0.4
        ...
        // Safe catch for transient unmounting 3D nodes
      }
    }
  }, [selectedObjectId, scene]);

  /**
   * Effect hook binding keyboard event listeners for WASD, Q/E, and Shift controls.
   */
  useEffect(() => {
