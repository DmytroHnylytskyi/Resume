"""
Pytest Test Configuration & Shared Async Fixtures for TerraScope Backend.

Provides:
    - In-memory SQLite async test database session fixture.
    - Test application client (httpx.AsyncClient with ASGITransport).
    - Database dependency override and automatic table setup/teardown.
"""
...
# policy), so tests inject a fixed test-only key BEFORE importing the app.
